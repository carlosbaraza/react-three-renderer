...
var _class; /* eslint no-debugger: 0 */
  ...
  function TextureDescriptor(react3RendererInstance) {
    ...
    _this.hasProp('slot', {
      ...
      update: function update(threeObject, slot, hasProperty) {
        var supportedSlots = ['map', 'bumpMap'];
  ...
  _createClass(TextureDescriptor, [{
    ...
    value: function setParent(texture, parentObject3D) {
      ...
      if (parentObject3D instanceof _three2.default.Material) {
        var _materialSlot = texture.userData._materialSlot;


        (0, _invariant2.default)(parentObject3D[_materialSlot] === null || parentObject3D[_materialSlot] === undefined, 'Parent already has a texture in slot "%s"', _materialSlot);
